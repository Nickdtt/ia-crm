...
MEETING_DURATION = 30   # Duração da reunião em minutos
BUFFER_BETWEEN = 15     # Intervalo entre reuniões em minutos
SLOT_INTERVAL = MEETING_DURATION + BUFFER_BETWEEN  # = 45 minutos

# Horários disponíveis por dia da semana (0=Segunda ... 4=Sexta)
# Cada entrada é uma lista de faixas: (hora_ini, min_ini, hora_fim, min_fim)
WEEKLY_SCHEDULE = {
    0: [(17, 0, 19, 0)],                                    # Seg: 17h-19h
    1: [(9, 0, 10, 0), (14, 0, 15, 0)],                     # Ter: 09-10h, 14-15h
    2: [(9, 0, 11, 0), (14, 0, 16, 0)],                     # Qua: 09-11h, 14-16h
    3: [(14, 0, 16, 0)],                                     # Qui: 14-16h
    4: [(9, 0, 12, 0), (14, 0, 15, 0), (16, 0, 18, 0)],     # Sex: 09-12h, 14-15h, 16-18h
}
...
async def get_available_slots(
    ...
    Horários variam por dia da semana (ver WEEKLY_SCHEDULE).
    Slots são gerados a cada SLOT_INTERVAL (45min = 30min reunião + 15min intervalo).
    ...
    # 1. Sem horários no fim de semana ou dias sem agenda
    ...
    # 3. Gera slots a partir das faixas do dia
    all_slots = []
    for (h_ini, m_ini, h_fim, m_fim) in WEEKLY_SCHEDULE[weekday]:
        slot_start = datetime.combine(target_date, time(h_ini, m_ini)).replace(tzinfo=BRAZIL_TZ)
        window_end = datetime.combine(target_date, time(h_fim, m_fim)).replace(tzinfo=BRAZIL_TZ)
        
        while slot_start + timedelta(minutes=MEETING_DURATION) <= window_end:
