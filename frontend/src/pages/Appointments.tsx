    ...
    <>
      <div className="w-full max-w-7xl mx-auto bg-white rounded-3xl shadow-xl border border-slate-100 p-6 flex flex-col h-[calc(100vh-60px)] overflow-hidden font-sans">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Agendamentos</h1>
            <p className="text-slate-500 mt-1 font-medium">Gestão da agenda</p>
          </div>

          <div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl border border-slate-100">
            <button 
              onClick={handleToday}
              className="px-4 py-2 text-sm font-bold text-slate-600 hover:bg-white hover:shadow-sm rounded-xl transition-all"
            >
              Hoje
            </button>
        ...
        <div className="flex-1 overflow-auto rounded-xl border border-slate-200 shadow-inner bg-slate-50/50">
          <table className="min-w-full border-separate border-spacing-0 bg-white">
            <thead className="bg-white">
              <tr>
                <th className="bg-white p-4 border-b border-r border-slate-200 sticky top-0 left-0 z-30 w-16 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]" />
                {weekDays.map((day) => (
                  <th
                    key={day.iso}
                    className="sticky top-0 z-20 border-b border-slate-200 bg-white px-2 py-4 min-w-[140px] text-center shadow-[0_2px_5px_-2px_rgba(0,0,0,0.05)]"
                  >
                    <div className="text-sm font-bold text-slate-700 uppercase tracking-wider">{day.label}</div>
                    <div className="text-xs font-medium text-slate-400 mt-0.5">{day.date}</div>
                  </th>
                ))}
              </tr>
            </thead>
